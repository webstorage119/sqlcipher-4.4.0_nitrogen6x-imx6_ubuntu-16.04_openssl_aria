...
public:
    explicit SQLiteResult(const QSqlCipherDriver* db);
    ~SQLiteResult();
...
{
public:
    inline QSqlCipherDriverPrivate() : access(0) {}
    sqlite3 *access;
...
void SQLiteResultPrivate::initColumns(bool emptyResultset)
{
    ...
    for (int i = 0; i < nCols; ++i) {
        ...
        // must use typeName for resolving the type to match QSqlCipher::record
...
SQLiteResult::SQLiteResult(const QSqlCipherDriver* db)
    : SqlCachedResult(db)
{
...
{
    const QSqlCipherDriver * sqlDriver = qobject_cast<const QSqlCipherDriver *>(driver());
    if (sqlDriver)
